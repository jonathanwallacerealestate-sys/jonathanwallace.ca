...
// (Routes and helpers moved to lib/routes/listing-photos.js — extracted
//  2026-04-24. We keep LISTING_PHOTOS_DIR + photoUpload here because
//  they're shared with other server code.)
// ─────────────────────────────────────────────

const LISTING_PHOTOS_DIR = path.join(DATA_DIR, '.listing-photos');
if (!fs.existsSync(LISTING_PHOTOS_DIR)) fs.mkdirSync(LISTING_PHOTOS_DIR, { recursive: true });
...
listingPhotosRoutes.register(app, {
  LISTING_PHOTOS_DIR, LISTING_FORMS_DIR, photoUpload, anthropic,
});
...
// parseGeoWarehouseText() moved to lib/parsers/geowarehouse.js (extracted 2026-04-24)
...
// ─────────────────────────────────────────────
// LISTING PRINT VIEW — returns a print-optimized HTML page that auto-opens
// the browser's print dialog. The user picks 'Save as PDF' → done.
// ─────────────────────────────────────────────

// esc() moved to lib/listing-print.js (extracted 2026-04-24)

// renderPrintHtml() moved to lib/listing-print.js (extracted 2026-04-24)

// GET /api/listing-form/:id/print — printable listing sheet; opens print dialog on load
app.get('/api/listing-form/:id/print', (req, res) => {
  const filePath = path.join(LISTING_FORMS_DIR, `${req.params.id}.json`);
  if (!fs.existsSync(filePath)) return res.status(404).send('Listing not found');
  try {
    const form = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.setHeader('Cache-Control', 'no-store');
    res.send(renderPrintHtml(form));
  } catch (err) {
    console.error('[ListingForm] print error:', err.message);
