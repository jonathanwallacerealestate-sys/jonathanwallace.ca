...
// Photo upload uses a separate multer instance with a larger per-file cap.
const photoUpload = multer({
  storage: multer.memoryStorage(),
  limits: { fileSize: 25 * 1024 * 1024, files: 60 },
});

function listingPhotosDir(propertyId) {
...
app.post('/api/listing-form/:id/photos', photoUpload.array('photos', 60), (req, res) => {
  ...
  const dir = listingPhotosDir(propertyId);
...
app.get('/api/listing-form/:id/photos', (req, res) => {
  const dir = listingPhotosDir(req.params.id);
  try {
    ...
    // Also return cached extract results if present
