...
const sidebarItems = [
  ...
  { id: "signings", label: "Signings", icon: FileSignature, badge: null },
...
// ─────────────────────────────────────────────
// ─────────────────────────────────────────────
// SIGNINGS SECTION — DocuSign In-Person Signing control panel
// Lives on the dashboard so Jonathan can start (or cancel) a signing
// session without opening the Listing Form. Lists every listing with
// its current signing state and inline action buttons.
// ─────────────────────────────────────────────
function SigningsSection() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (j.success) {
        ...
        setError(j.error || 'Could not load listings');
      }
  ...
  const startSigning = async (listing) => {
    ...
    if (!confirm(`Start in-person signing for ${listing.address || 'this listing'}?\n\nThis creates a DocuSign envelope (RECO + MLS Listing Agreement) pre-filled with the seller info.`)) return;
    ...
    try {
      ...
      if (j.success) {
        ...
        alert(`Could not start signing: ${j.error || 'unknown'}${j.detail ? `\n${j.detail}` : ''}`);
      }
    } catch (e) {
      alert(`Could not start signing: ${e.message}`);
    }
    setBusyId(null);
  };

  const cancelSigning = async (listing) => {
    if (!confirm(`Cancel the in-person signing session for ${listing.address || 'this listing'}?\n\nThis clears the in-progress state. The DocuSign envelope (if one was created) will remain in DocuSign.`)) return;
    setBusyId(listing.propertyId);
    try {
      ...
      if (j.success) {
        ...
        alert(`Could not cancel: ${j.error || 'unknown'}`);
      }
    } catch (e) {
  ...
  if (loading) return <Card><div style={{ padding: 40, textAlign: 'center', color: '#6b7280' }}>Loading…</div></Card>;
  if (error) return <Card><div style={{ padding: 20, color: '#b91c1c' }}>Error: {error}</div></Card>;

  const open = listings.filter(l => l.status !== 'closed');
  const stats = {
    notStarted: open.filter(l => !l.signingStatus || l.signingStatus === 'not_started').length,
    inProgress: open.filter(l => l.signingStatus === 'in_progress').length,
    complete: open.filter(l => l.signingStatus === 'complete').length,
  };
    ...
    <Card>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 9, fontWeight: 700, color: '#fff', background: '#2563eb', padding: '2px 6px', borderRadius: 3 }}>DOCUSIGN</span>
          </div>
          <div style={{ fontSize: 12, color: '#6b7280', marginTop: 2 }}>
            Start or cancel an in-person signing session for any listing. RECO Information Guide + MLS Listing Agreement are pre-filled and opened on this device.
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 16 }}>
        <div style={{ padding: '12px 10px', borderRadius: 10, background: '#eff6ff', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 10, color: '#2563eb', fontWeight: 600 }}>Awaiting Signing</div>
        </div>
              ...
              <div key={l.propertyId} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: '#6b7280' }}>
                    {l.sellerName || '(no seller)'}{l.sellerEmail ? ` · ${l.sellerEmail}` : ''}
                  </div>
                </div>
                {status === 'complete' && (
                  <span style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 6, border: '1px solid #059669', background: '#ecfdf5', color: '#065f46', fontSize: 11, fontWeight: 700 }}>
                ...
                {status === 'in_progress' && (
                  <>
                    {l.docusignEnvelopeUrl && (
                      <button onClick={() => window.open(l.docusignEnvelopeUrl, '_blank', 'noopener,noreferrer')} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 8, border: '1px solid #d97706', background: '#fff7ed', color: '#92400e', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                        <Loader2 size={12} style={{ animation: 'spin 1s linear infinite' }} /> Reopen Envelope
                      </button>
                    )}
                    {!l.docusignEnvelopeUrl && (
                      <span style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 6, border: '1px solid #d97706', background: '#fff7ed', color: '#92400e', fontSize: 11, fontWeight: 700 }}>
                        <Loader2 size={12} style={{ animation: 'spin 1s linear infinite' }} /> In Progress
                      </span>
                    )}
                    <button onClick={() => cancelSigning(l)} disabled={isBusy} style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid #d1d5db', background: '#fff', color: '#6b7280', fontSize: 11, fontWeight: 600, cursor: isBusy ? 'not-allowed' : 'pointer' }}>
                      {isBusy ? 'Cancelling…' : 'Cancel Signing'}
                    </button>
                  </>
                )}
                {(status === 'not_started' || status === '') && (
                  <button onClick={() => startSigning(l)} disabled={isBusy} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 14px', borderRadius: 8, border: '1px solid #2563eb', background: '#eff6ff', color: '#1e40af', fontSize: 11, fontWeight: 600, cursor: isBusy ? 'not-allowed' : 'pointer' }}>
                    <FileSignature size={12} /> {isBusy ? 'Starting…' : 'Start Signing'}
                  </button>
                )}
              </div>
            );
...
function PriceRangeSlider({ cma, onPatch }) {
  ...
  // Geometry for the SVG bullet chart. Outer band = full comp price range,
  // inner gold band = CMA Low–High, dark vertical tick = Most Likely. Every
  // x-position is proportional to actual prices, so the geometry COMMUNICATES
  // the data instead of being decorative. Replaces the prior fixed-isosceles
  // triangle which was the same shape regardless of the underlying spread.
  const BC_W = 480;
  const BC_H = 220;
  const BAR_X = 40;
  const BAR_W = 400;
  ...
  const hasComps = compPrices.length > 0;
  const compMin = hasComps ? Math.min(...compPrices) : sliderMin;
  const compMax = hasComps ? Math.max(...compPrices) : sliderMax;
  const compSpan = Math.max(1, compMax - compMin);
  const px = (p) => BAR_X + BAR_W * Math.max(0, Math.min(1, (p - compMin) / compSpan));

  const lowX = px(dragLow);
  const highX = px(dragHigh);
  const midX = px(dragMid);

  // Clamp text-anchor positions so labels don't get clipped at the SVG edges
  const clampLabelX = (x) => Math.max(BAR_X + 8, Math.min(BAR_X + BAR_W - 8, x));
  const midLabelX = clampLabelX(midX);
  const lowLabelX = clampLabelX(lowX);
  const highLabelX = clampLabelX(highX);
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg,#fffdf5,#fefce8)', border: '1px solid #fde68a', borderRadius: 12, padding: 20, marginBottom: 16 }}>
      ...
      {/* SVG bullet chart — outer band is the full comp price range, inner
          gold band is the CMA Low–High, the dark vertical tick is Most Likely.
          All x-positions scale proportionally to actual prices, so the
          geometry reflects the spread instead of being a fixed shape. */}
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 18 }}>
        <svg viewBox={`0 0 ${BC_W} ${BC_H}`} width="100%" style={{ maxWidth: 480, display: 'block' }} xmlns="http://www.w3.org/2000/svg">
          {/* Headline ML number above the bar */}
          <text x={midLabelX} y="50" textAnchor="middle" fontSize="28" fontWeight="800" fill="#0f172a">{fmt(dragMid)}</text>
          <text x={midLabelX} y="68" textAnchor="middle" fontSize="10" fontWeight="700" fill="#92400e" letterSpacing="0.6" style={{ textTransform: 'uppercase' }}>Most Likely</text>

          {/* Outer band — full comp range */}
          <rect x={BAR_X} y={BAR_Y - OUTER_H / 2} width={BAR_W} height={OUTER_H} fill="#fef3c7" stroke="#e5d4a3" strokeWidth="0.5" />

          {/* Inner band — CMA Low to High */}
          <rect x={lowX} y={BAR_Y - INNER_H / 2} width={Math.max(2, highX - lowX)} height={INNER_H} fill="#c8a96e" />

          {/* Most Likely tick — extends above and below the outer band */}
          <line x1={midX} y1={BAR_Y - OUTER_H / 2 - 12} x2={midX} y2={BAR_Y + OUTER_H / 2 + 12} stroke="#0f172a" strokeWidth="3" strokeLinecap="round" />

          {/* Low label below */}
          <text x={lowLabelX} y={BAR_Y + OUTER_H / 2 + 34} textAnchor="middle" fontSize="14" fontWeight="700" fill="#78350f">{fmt(dragLow)}</text>
          <text x={lowLabelX} y={BAR_Y + OUTER_H / 2 + 48} textAnchor="middle" fontSize="9" fontWeight="700" fill="#92400e" letterSpacing="0.6" style={{ textTransform: 'uppercase' }}>Low</text>

          {/* High label below */}
          <text x={highLabelX} y={BAR_Y + OUTER_H / 2 + 34} textAnchor="middle" fontSize="14" fontWeight="700" fill="#78350f">{fmt(dragHigh)}</text>
          <text x={highLabelX} y={BAR_Y + OUTER_H / 2 + 48} textAnchor="middle" fontSize="9" fontWeight="700" fill="#92400e" letterSpacing="0.6" style={{ textTransform: 'uppercase' }}>High</text>

          {/* Comp range bookends — only when we have actual comps */}
          {hasComps ? (
            <>
              <text x={BAR_X} y={BC_H - 6} textAnchor="start" fontSize="9" fill="#92400e">Comp range {fmt(compMin)}</text>
              <text x={BAR_X + BAR_W} y={BC_H - 6} textAnchor="end" fontSize="9" fill="#92400e">{fmt(compMax)}</text>
            </>
          ) : null}
        </svg>
      </div>
